...
class Auth0 extends EventEmitter {
  ...
  getIntlMessage = (id, fallback) => {
    const locale = store.getState()?.LanguageSwitcher?.language?.locale || "tr";
    return AppLocale[locale]?.messages?.[id] || fallback;
  };
  ...
  handleAuthentication = () => {
    ...
    this.lock.on("authorization_error", (error) =>
      notification(
        "error",
        this.getIntlMessage(
          "authentication.auth0.wrongCredentials",
  ...
  setSession = (authResult) => {
    ...
    localStorage.setItem("id_token", authResult.isLoggedIn);
