...
const AddPoll = ({ refreshList }) => {
  ...
  const onFinish = async (values) => {
    ...
    if (questions.length === 0) {
      messageApi.open({
        type: "warning",
        content: intl.formatMessage({ id: "polls.add.questionsMinWarning" }),
      });
      return;
    }
    ...
    try {
      ...
      messageApi.open({
        type: "success",
        content: intl.formatMessage({ id: "polls.add.success" }),
      });
      form.resetFields();
      refreshList();
      ...
      const errMsg =
        ...
        err?.message ||
        intl.formatMessage({ id: "polls.add.error" });
  ...
  return (
    <div>
      ...
      >
        {intl.formatMessage({ id: "polls.add.button" })}
      </Button>
      <Modal
        width={700}
        title={intl.formatMessage({ id: "polls.add.title" })}
        open={isModalOpen}
      ...
      >
        {!deptLoading && bolumDepartments.length === 0 ? (
          <Alert
            ...
            message={intl.formatMessage({ id: "polls.form.bolumMissing" })}
          />
        ) : null}
        <Form
          form={form}
        ...
        >
          ...
          <Form.Item wrapperCol={{ offset: 6, span: 18 }} style={{ marginTop: 12, marginBottom: 0 }}>
            <Button type="primary" htmlType="submit" loading={apiProgress}>
              {intl.formatMessage({ id: "polls.form.submitCreate" })}
            </Button>
          </Form.Item>
        </Form>
      </Modal>
